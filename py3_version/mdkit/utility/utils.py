...
def check_amber_version():
    if os.environ.get('AMBERHOME'):
        ...
        if not docfile:
            print("⚠️  Amber version file not found in $AMBERHOME/doc. Proceeding with default version.")
            return "unknown"
        ...
        try:
            ...
            raise ValueError("Amber version not detected from doc filename")
    else:
...
def get_box_dimensions(pdbfile, mask=[]):
    ...
    for line in pdbf.next()['ATOM']:
        if (mask and line[2].strip() in mask) or not mask:
            coords.append(list(map(float,line[4:])))
    coords = np.array(coords)
...
def extract_tgtmd_lines(parsed_string):
    ...
    if args_tgtmd:
        tgtmd_lines = "\nitgtmd=1"
        for key, value in args_tgtmd.items():
